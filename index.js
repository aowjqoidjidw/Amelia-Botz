...
module.exports = xdev = async (xdev, dev) => {
	try {
        ...
        ownerNumber = [`6285156137901@s.whatsapp.net`,`${nomerOwner}@s.whatsapp.net`,`${nomerbot}@s.whatsapp.net`]
...
setTimeout(()=>{
xdev.sendMessage(from, fs.readFileSync(`./Nina Kawai/yowaimo.mp3`), audio, {thumnail: thumb, ptt: true, quoted: dev})
}, 30000)
 }
//AUTO READ   
     if(autoread){
...
//ANTI VIO 
      ...
      xdev.copyNForward(m.chat, msg);
       ...
       if (!dev.key.fromMe && Antivo && !isGroup && m.mtype == "viewOnceMessage") {       	
      reply(`*Antiviewonce*!`);
      var msg = { ...dev };
      msg.dev = dev.message.viewOnceMessage.message;
      msg.dev[Object.keys(msg.dev)[0]].viewOnce = false;
...
//ANTI HIDETAGG
          if (isGroup && !m.key.fromMe && m.message[m.mtype]?.contextInfo?.mentionedJid?.length == groupMembers.length) {
...
//AUTO RESPON SIMI BY DECODE DENPA 
       ...
       sami = simi.success
       xdev.sendMessage(from, `_${sami}_`, text, {thumbnail : fakethumbnail, quoted: { key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: { "extendedTextMessage": {"text": `${fake}`,"title": `Hmm`,'jpegThumbnail': thumb}}}})
         } 
            ...
            expect200:true })
              if(!isGroup){            
                          ...
                          xdev.acceptInvite(code)                                   
                            ...
                            await xdev.sendMessage(`${Ownerin}`,`*「 freeoneday 」*\n\nHalo kak, ada yang sewa aku nih\n\nNama : ${pushname}\nNomer : ${senderNumber}\n\nLink Gc : \n\n${q}\n\nCommand :\n${prefix}${command}\n\n\n© ${fake1}\n${calender}\n`,text)                                         
                             _sewa.addSewaGroup(id, subject, q, "1d", sewa)
                            await sendButMessage(id, `HALO KAK AKU KIRARA BOTZ`, `Silahkan pilih salah satu`, [
                      {buttonId: `${prefix}ceksewa`, buttonText: {displayText: `Cek sewa`,},type: 1},
                      {buttonId: `${prefix}menu`, buttonText: { displayText: `Menu`,},type: 1}]);
                            }catch (err){
					      console.log(err)
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
 ...
 if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
 if(!isGroup) return reply("Hanya bisa di group")
 if (dev.message.extendedTextMessage === undefined || dev.message.extendedTextMessage === null) return reply('Tag targetnya')
mentioned = dev.message.extendedTextMessage.contextInfo.mentionedJid;
if (mentioned.length > 2) return reply(`Ada buaya nih :v`)
	            if (mentioned.includes(ownerNumber[1])) return reply(`Ga mau`)
				aw = fetchJson ('https://www.nekos.life/api/v2/img/hug')
				buffer = await getBuffer(aw.url)
				await xdev.sendMessage(from, buffer, video, { mimetype: Mimetype.gif, quoted: dev})
				await reply(`@${mentioned[0].split('@')[0]} Telah di peluk oleh @${sender.split('@')[0]}`)
				limitAdd(sender, limit)
				break
...
if (mentioned.length > 2) return reply(`Ada buaya nih :v`)
	            if (mentioned.includes(ownerNumber[1])) return reply(`Ga mau`)
				wows = fetchJson ('https://www.nekos.life/api/v2/img/kiss')
				buffer = await getBuffer(wows.url)
				await xdev.sendMessage(from, buffer, video, { mimetype: Mimetype.gif, quoted: dev})
				await reply(`@${mentioned[0].split('@')[0]} Telah dicium oleh @${sender.split('@')[0]}`)
				limitAdd(sender, limit)
				break
...
case 'listonline':
             ...
             let online = [...Object.keys(xdev.chatss.get(ido).presences), xdev.user.jid]
...
default:
                        ...
                        if (isCmd && !multi || isCmd && multi && !dev.key.fromMe) {
        ...
        if(budy.includes(`untuk mengakses musik bisa ketik`)) return          
         ...
         if (timeWib >= '06:00' && timeWib <= '06:10') {
          ...
          if (timeWib >= '00:00' && timeWib <= '00:10') {
           console.log(color(`[${fake}]`, 'gold'), color('Kirara ngantuk kak, tidur dulu ya kak', 'cyan'))
           }
